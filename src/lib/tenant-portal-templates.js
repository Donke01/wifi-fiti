...
function attachTenantPortalTemplateRoutes(app, { businessAuth, db }) {
  ...
  const validate = body => {
    ...
    return { name, layout, accentColor: accent.toUpperCase(), welcomeMessage: clean(body.welcomeMessage || body.welcome_message, 140), showPackages: body.showPackages === false ? 0 : 1, showUtilities: body.showUtilities === false ? 0 : 1 };
  };
  const publicTemplate = row => row && ({ id: row.id, name: row.name, layout: row.layout, accentColor: row.accent_color, welcomeMessage: row.welcome_message, showPackages: Boolean(row.show_packages), showUtilities: Boolean(row.show_utilities), active: Boolean(row.active), createdAt: row.created_at, updatedAt: row.updated_at });
  ...
  app.post('/api/business/portal-templates', operator((req, res, business) => {
    ...
    db.prepare(`INSERT INTO tenant_portal_templates(id,business_id,name,layout,accent_color,welcome_message,show_packages,show_utilities,active) VALUES(?,?,?,?,?,?,?,?,?)`).run(templateId, business.id, values.name, values.layout, values.accentColor, values.welcomeMessage, values.showPackages, values.showUtilities, 0);
  ...
  app.patch('/api/business/portal-templates/:templateId', operator((req, res, business) => {
    ...
    db.prepare(`UPDATE tenant_portal_templates SET name=?,layout=?,accent_color=?,welcome_message=?,show_packages=?,show_utilities=?,updated_at=datetime('now') WHERE id=? AND business_id=?`).run(values.name, values.layout, values.accentColor, values.welcomeMessage, values.showPackages, values.showUtilities, current.id, business.id);
